...
import type { AdminSupportTicketListItem } from "./data";
...
export default function SupportTable({ tickets }: { tickets: AdminSupportTicketListItem[] }) {
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 border border-[#e5e7eb] bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="shrink-0 text-sm text-[#6b7280]">
          Toplam <span className="font-semibold text-[#111111]">{tickets.length}</span> talep
          {filtered.length !== tickets.length && <span> · {filtered.length} sonuç gösteriliyor</span>}
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="border border-[#e5e7eb] bg-white">
          <EmptyState
            ...
            description="Arama veya filtre kriterlerine uyan bir destek talebi yok."
          />
        </div>
      ) : (
        <div className="overflow-x-auto border border-[#e5e7eb] bg-white">
          <table className="w-full min-w-[720px] border-collapse text-left text-sm">
            ...
            <tbody>
              {filtered.map((ticket) => (
                <tr key={ticket.id} className="border-b border-[#f0f0f0] last:border-b-0 hover:bg-[#F7F7F8]">
                  <td className="px-4 py-3">
                    <Link href={`/admin/support/${ticket.id}`} className="font-medium text-[#111111] hover:text-[#2563EB]">
                      {ticket.subject}
                    </Link>
                  </td>
                  <td className="px-4 py-3 text-[#4B5563]">{ticket.requesterName}</td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 text-[#4B5563]">{formatDate(ticket.createdAt)}</td>
                </tr>
              ))}
            </tbody>
          </table>
